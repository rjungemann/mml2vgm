...
use mml2vgm::{
    compiler::compiler::MmlCompiler, player::VgmPlayer, utils::wav::write_wav, CompileInfo,
    CompileOptions, CompileResult, MmlError, MmlResult, OutputFormat, SoundChip,
};
...
fn success(msg: &str, use_color: bool) {
    println!(
        "{}",
        colorize(&format!("✓ {}", msg), colors::GREEN, use_color)
    );
}
...
fn error_msg(msg: &str, use_color: bool) {
    eprintln!(
        "{}",
        colorize(&format!("✗ {}", msg), colors::RED, use_color)
    );
}
...
fn info_msg(msg: &str, use_color: bool) {
    println!(
        "{}",
        colorize(&format!("ℹ {}", msg), colors::CYAN, use_color)
    );
}
...
fn warning_msg(msg: &str, use_color: bool) {
    println!(
        "{}",
        colorize(&format!("⚠ {}", msg), colors::YELLOW, use_color)
    );
}
...
fn batch_compile(dir: &Path, format: OutputFormat, use_color: bool, quiet: bool) -> MmlResult<()> {
    if !dir.is_dir() {
        return Err(MmlError::UnsupportedCommand(format!(
            "Not a directory: {}",
            dir.display()
        )));
    }
    ...
    if !quiet {
        info_msg(
            &format!("Compiling {} files from {}", files.len(), dir.display()),
            use_color,
        );
    }
    ...
    for (idx, input) in files.iter().enumerate() {
        ...
        match compiler.compile(input) {
            ...
            Err(e) => {
                ...
                error_msg(
                    &format!("Failed to compile {}: {}", input.display(), e),
                    use_color,
                );
                error_count += 1;
            }
    ...
    if !quiet {
        if error_count == 0 {
            success(
                &format!(
                    "Batch compilation complete: {} files compiled",
                    success_count
                ),
                use_color,
            );
...
fn list_supported_chips() {
    ...
    let categories: Vec<(&str, Vec<SoundChip>)> = vec![
        (
            "Mega Drive / Genesis",
            vec![SoundChip::YM2612, SoundChip::YM2612X, SoundChip::YM2612X2],
        ),
        (
            "PSG / SSG",
            vec![SoundChip::SN76489, SoundChip::SN76489X2, SoundChip::AY8910],
        ),
        ...
        (
            "Console Chips",
            vec![SoundChip::NES, SoundChip::DMG, SoundChip::VRC6],
        ),
...
fn list_supported_formats() {
    ...
    println!();
    println!(
        "  - vgm  [{}] - Standard VGM format",
        OutputFormat::VGM.support_tier()
    );
    println!(
        "  - xgm  [{}] - XGM format (Mega Drive)",
        OutputFormat::XGM.support_tier()
    );
    println!(
        "  - xgm2 [{}] - XGM2 format (Mega Drive with extended features)",
        OutputFormat::XGM2.support_tier()
    );
    println!(
        "  - zgm  [{}] - ZGM format (Extended VGM with YM2609 and MIDI support)",
        OutputFormat::ZGM.support_tier()
    );
    println!(
        "  - mid  [{}] - Standard MIDI File format",
        OutputFormat::MID.support_tier()
    );
    println!();
    println!("Default: vgm");
}
...
fn print_diagnostic(path: Option<&Path>, err: &MmlError, use_color: bool) {
    match err {
        ...
        MmlError::UnsupportedChip(name) => {
            ...
            info_msg(
                "Valid chip names: YM2612, SN76489, YM2608, YM2151, YM3812, AY8910, and others",
                use_color,
            );
...
fn print_stats(info: &CompileInfo, use_color: bool) {
    ...
    println!(
        "{}",
        colorize("Compilation Statistics:", colors::BOLD, use_color)
    );
    println!("  Parts: {}", info.part_count);
    println!("  Commands: {}", info.command_count);
    println!(
        "  Duration: {:.2} seconds ({} samples)",
        info.duration_seconds, info.duration_samples
    );
...
fn run_validate(args: &Args, use_color: bool) -> MmlResult<()> {
    ...
    if !args.quiet {
        info_msg(
            &format!("Validating MML file: {}", input.display()),
            use_color,
        );
    }
...
fn run_compile(args: &Args, use_color: bool) -> MmlResult<CompileResult> {
    ...
    if !args.quiet {
        info_msg(
            &format!("Compiling MML file: {}", input.display()),
            use_color,
        );
    ...
    if output_path.to_string_lossy() != "-" {
        std::fs::write(&output_path, &result.data).map_err(|e| {
            MmlError::UnsupportedCommand(format!("Failed to write output file: {}", e))
        })?;
        if !args.quiet {
            info_msg(
        ...
        // Write to stdout
        std::io::stdout().write_all(&result.data).map_err(|e| {
            MmlError::UnsupportedCommand(format!("Failed to write to stdout: {}", e))
        })?;
    }
    ...
    if let Some(ref input) = args.input {
        ...
        if is_compiled_audio_file(input) {
            if !args.play && args.export_wav.is_none() {
                error_msg(
                    "Input is a compiled audio file. Use --play or --export-wav.",
                    use_color,
                );
                process::exit(1);
            }
...
mod tests {
    ...
    fn test_format_arg_conversion() {
        assert_eq!(OutputFormat::from(FormatArg::Vgm), OutputFormat::VGM);
        assert_eq!(OutputFormat::from(FormatArg::Xgm), OutputFormat::XGM);
    }
    ...
    fn test_output_path_determination() {
        ...
        let path = determine_output_path(Some(&input), None, OutputFormat::VGM).unwrap();

        assert_eq!(path, PathBuf::from("test.vgm"));

        let path = determine_output_path(Some(&input), None, OutputFormat::XGM).unwrap();

        assert_eq!(path, PathBuf::from("test.xgm"));

        let path = determine_output_path(Some(&input), None, OutputFormat::XGM2).unwrap();

        assert_eq!(path, PathBuf::from("test.xgm2"));

        let custom = PathBuf::from("custom.vgm");
        let path = determine_output_path(Some(&input), Some(&custom), OutputFormat::VGM).unwrap();
