...
impl YM2608 {
    ...
    fn get_ssg_output(&self) -> (f32, f32) {
        ...
        for ch in &self.ssg_channels {
            if ch.frequency > 0 && ch.volume > 0 {
                ...
                let square = if (ch.phase / half.max(1)).is_multiple_of(2) {
                    1.0f32
                } else {
                    -1.0f32
                };
    ...
    fn adpcm_b_decode_nibble(&mut self, nibble: u8) -> f32 {
        ...
        if (nibble & 8) != 0 {
            self.adpcm_b.adpcm_predictor =
                (self.adpcm_b.adpcm_predictor - delta).clamp(-32768, 32767);
        } else {
            self.adpcm_b.adpcm_predictor =
                (self.adpcm_b.adpcm_predictor + delta).clamp(-32768, 32767);
        }
    ...
    fn get_adpcm_b_sample(&mut self) -> f32 {
        if !self.adpcm_b.active || self.adpcm_b_rom.is_empty() {
            return 0.0;
        }
        let raw_step = if self.adpcm_b.delta_n == 0 {
            0x100u32
        } else {
            self.adpcm_b.delta_n as u32
        };
        let divisor = if self.adpcm_b.prescaler == 0 {
            1u32
        } else {
            self.adpcm_b.prescaler as u32
        };
        ...
        while self.adpcm_b.frac >= 0x100 {
            ...
            let end = end_by_reg
                .min(end_by_limit)
                .min(self.adpcm_b_rom.len() as u32);
    ...
    fn apply_register(&mut self, port: u8, addr: u8, data: u8) {
        ...
        if port == 0 {
            match addr {
                ...
                // SSG registers
                0x00 => {
                    self.ssg_channels[0].frequency =
                        (self.ssg_channels[0].frequency & 0xF00) | data as u16
                }
                0x01 => {
                    self.ssg_channels[0].frequency =
                        (self.ssg_channels[0].frequency & 0x0FF) | ((data as u16 & 0x0F) << 8)
                }
                0x02 => {
                    self.ssg_channels[1].frequency =
                        (self.ssg_channels[1].frequency & 0xF00) | data as u16
                }
                0x03 => {
                    self.ssg_channels[1].frequency =
                        (self.ssg_channels[1].frequency & 0x0FF) | ((data as u16 & 0x0F) << 8)
                }
                0x04 => {
                    self.ssg_channels[2].frequency =
                        (self.ssg_channels[2].frequency & 0xF00) | data as u16
                }
                0x05 => {
                    self.ssg_channels[2].frequency =
                        (self.ssg_channels[2].frequency & 0x0FF) | ((data as u16 & 0x0F) << 8)
                }
                ...
                0x10 => {
                    ...
                    for (i, ch) in self.adpcm_a_channels.iter_mut().enumerate() {
                        if (data >> i) & 1 == 1 {
                            ch.key_on = key_on;
                            if key_on {
                                ch.position = ch.start_addr * 32;
                            }
                        }
                    }
                }
                0x11 => {
                    self.adpcm_a_master_vol = data & 0x3F;
                }
                0x18..=0x1D => {
                ...
                _ => {
                    self.apply_fm_register(ch_base, addr, data);
                }
            }
        } else {
            match addr {
                ...
                0x03 => {
                    self.adpcm_b.start_addr =
                        (self.adpcm_b.start_addr & 0x00FF) | ((data as u32) << 8);
                }
                0x04 => {
                    self.adpcm_b.end_addr = (self.adpcm_b.end_addr & 0xFF00) | data as u32;
                }
                0x05 => {
                    self.adpcm_b.end_addr = (self.adpcm_b.end_addr & 0x00FF) | ((data as u32) << 8);
                }
                0x06 => {
                    self.adpcm_b.limit_addr = (self.adpcm_b.limit_addr & 0xFF00) | data as u32;
                }
                0x07 => {
                    self.adpcm_b.limit_addr =
                        (self.adpcm_b.limit_addr & 0x00FF) | ((data as u32) << 8);
                }
                0x08 => {
                ...
                0x0A => {
                    self.adpcm_b.level = data;
                }
                0x10 => {
                    self.adpcm_b.prescaler = (self.adpcm_b.prescaler & 0xFF00) | data as u16;
                }
                0x11 => {
                    self.adpcm_b.prescaler =
                        (self.adpcm_b.prescaler & 0x00FF) | ((data as u16) << 8);
                }
        ...
        match addr {
            ...
            0x30..=0x3F => {
                if let Some((ch, op)) = opn_ch_op(addr - 0x30) {
                    let c = ch_base + ch;
                    if c < 6 {
                        self.fm_channels[c].operators[op].mult = data & 0x0F;
                    }
                }
            }
            ...
            0x40..=0x4F => {
                if let Some((ch, op)) = opn_ch_op(addr - 0x40) {
                    let c = ch_base + ch;
                    if c < 6 {
                        self.fm_channels[c].operators[op].total_level = data & 0x7F;
                    }
                }
            }
...
impl SoundChipEmulator for YM2608 {
    ...
    fn load_pcm_data(&mut self, block_type: u8, data: &[u8]) {
        match block_type {
            0x81 => {
                self.adpcm_b_rom = data.to_vec();
            }
            0x82 => {
                self.adpcm_a_rom = data.to_vec();
            }
            _ => {}
        }
    }
    ...
    fn generate_samples(&mut self, buffer: &mut [f32], sample_rate: u32) {
        ...
        for frame in buffer.chunks_mut(2) {
            ...
            let adpcm_b_left = if self.adpcm_b.pan_left {
                adpcm_b_sample
            } else {
                0.0
            };
            let adpcm_b_right = if self.adpcm_b.pan_right {
                adpcm_b_sample
            } else {
                0.0
            };
...
mod tests {
    ...
    fn test_ym2608_adpcm_b_limit_address() {
        ...
        chip.load_pcm_data(0x81, &[0x77u8; 128]);
        ...
        assert!(
            chip.adpcm_b.active,
            "channel should still be active after one half-step"
        );
    }
    ...
    fn test_ym2608_adpcm_a_address_registers() {
        ...
        assert_eq!(
            chip.adpcm_a_channels[0].position,
            0x0210 * 32,
        ...
        assert!(
            !chip.adpcm_a_channels[0].key_on,
