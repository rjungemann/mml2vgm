...
//!
//! The Y8950 is an OPL variant with ADPCM-B playback used in MSX music cartridges.
//! It combines 9 FM channels (same register map as YM3526/YM3812) with ADPCM.
//!
//! OPL register map (same as YM3526):
//! - 0x20-0x35: Operator AM/VIB/EG/KSR/MULT
//! - 0x40-0x55: Operator KSL/TL
...
/// Y8950 chip emulator with 9 FM channels and ADPCM-B
pub struct Y8950 {
    clock_rate: u32,
    sample_rate: u32,
    regs: [u8; 0x100],
...
impl SoundChipEmulator for Y8950 {
    ...
    fn load_pcm_data(&mut self, block_type: u8, data: &[u8]) {
        if block_type == 0x01 {
            let len = data.len().min(self.adpcm_memory.len());
            self.adpcm_memory[..len].copy_from_slice(&data[..len]);
        }
    }
    ...
    fn clock(&mut self) {
        if self.adpcm_playing {
            ...
            if self.adpcm_address > self.adpcm_stop && self.adpcm_stop > 0 {
    ...
    fn generate_samples(&mut self, buffer: &mut [f32], sample_rate: u32) {
        ...
        for frame in buffer.chunks_mut(2) {
            self.accumulated_cycles += self.clock_rate as f32 / sample_rate as f32;
            ...
            let (adl, adr) = self.get_adpcm_output();
            frame[0] = ((left / 9.0) + adl).clamp(-1.0, 1.0);
            frame[1] = ((right / 9.0) + adr).clamp(-1.0, 1.0);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_y8950_generate_samples_active() {
        let mut chip = Y8950::new();
        chip.write(0xA0, 0x57);
        chip.write(0xB0, 0x31);
