...
impl YM2203 {
    ...
    fn advance_ssg_phases(&mut self, sample_rate: u32) {
        for ch in 0..3 {
            if self.ssg_channels[ch].period == 0 { continue; }
    ...
    fn get_ssg_output(&self, ch: usize) -> f32 {
        ...
        if !tone_active || ssg.volume == 0 || ssg.period == 0 { return 0.0; }
...
impl SoundChipEmulator for YM2203 {
    ...
    fn write(&mut self, addr: u8, data: u8) {
        ...
        match addr {
            ...
            0x00 => self.ssg_channels[0].period = (self.ssg_channels[0].period & 0xF00) | data as u16,
            0x01 => self.ssg_channels[0].period = (self.ssg_channels[0].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            0x02 => self.ssg_channels[1].period = (self.ssg_channels[1].period & 0xF00) | data as u16,
            0x03 => self.ssg_channels[1].period = (self.ssg_channels[1].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            0x04 => self.ssg_channels[2].period = (self.ssg_channels[2].period & 0xF00) | data as u16,
            0x05 => self.ssg_channels[2].period = (self.ssg_channels[2].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            ...
            0x30..=0x3F => { if let Some((ch, op)) = opn_ch_op(addr - 0x30) { self.fm_channels[ch].operators[op].mult = data & 0x0F; } }
            0x40..=0x4F => { if let Some((ch, op)) = opn_ch_op(addr - 0x40) { self.fm_channels[ch].operators[op].total_level = data & 0x7F; } }
...
mod tests {
    ...
    fn test_ym2203_generate_samples_fm_active() {
        ...
        assert!(buffer.iter().any(|&s| s != 0.0), "FM channel must produce output");
    }
