...
const AY_VOL_TABLE: [f32; 16] = [
    0.000, 0.012, 0.016, 0.024, 0.033, 0.046, 0.064, 0.090,
    0.127, 0.179, 0.253, 0.358, 0.506, 0.715, 1.010, 1.000,
];
...
impl AY8910 {
    ...
    fn apply_reg(&mut self, addr: u8, data: u8) {
        ...
        match addr {
            ...
            0x01 => self.channels[0].period = (self.channels[0].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            0x02 => self.channels[1].period = (self.channels[1].period & 0xF00) | data as u16,
            0x03 => self.channels[1].period = (self.channels[1].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            0x04 => self.channels[2].period = (self.channels[2].period & 0xF00) | data as u16,
            0x05 => self.channels[2].period = (self.channels[2].period & 0x0FF) | ((data as u16 & 0x0F) << 8),
            0x06 => self.noise_period = data & 0x1F,
            0x07 => self.mixer = data,
            0x08 => { self.channels[0].volume = data & 0x0F; self.channels[0].env_mode = (data & 0x10) != 0; }
            0x09 => { self.channels[1].volume = data & 0x0F; self.channels[1].env_mode = (data & 0x10) != 0; }
            0x0A => { self.channels[2].volume = data & 0x0F; self.channels[2].env_mode = (data & 0x10) != 0; }
            ...
            if !self.env_hold {
                ...
                if self.env_step >= 16 {
                    if self.env_continue {
                        if self.env_alternate { self.env_attack = !self.env_attack; }
                        self.env_step = if self.env_attack { 0 } else { 15 };
                    } else {
                        self.env_step = 0;
                        self.env_hold = true;
...
impl SoundChipEmulator for AY8910 {
    fn name(&self) -> &'static str { "AY-3-8910" }
    ...
    fn generate_samples(&mut self, buffer: &mut [f32], sample_rate: u32) {
        ...
        for frame in buffer.chunks_mut(2) {
            ...
            for ch in 0..3 {
                ...
                let tone_out = if self.channels[ch].phase_acc < PI { 1.0f32 } else { -1.0 };

                let combined = match (tone_disable, noise_disable) {
                    (true, true) => 1.0,
                    (false, true) => tone_out,
                    (true, false) => self.noise_out(),
                    (false, false) => if tone_out > 0.0 && self.noise_out() > 0.0 { 1.0 } else { -1.0 },
                };
